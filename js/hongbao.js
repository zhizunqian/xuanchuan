function Hongbao(str){
		//默认参数
		this.canshu={
			coinSrc:"./img/hb.png",     //金币图片地址
			// audioSrc:"http://download.taobaocdn.com/freedom/26370/media/shake.mp3",	//金币音频地址
			coinWidth:67.5,           //金币宽度
			coinHeight:112.5,          //金币高度
			density:30,
		};
		this.settings=this._extendDeep(this.canshu,str);   //深拷贝
		...
		_startCacheCanvasAnim:function(){
			...
			for(var i=0;i<_self.density;i++){
				coinAttrArr[i]={
					...
					reDownSpeed:Math.random()*100+100,                    //存储金币弹起后重新降落的速度
					reDownHDelta:Math.random()*150+350,                    //存储金币弹起的高度参数，随机值250~350之间
				...
				for(var i=0;i<_self.density;i++){
					...
					if(coinAttrArr[i].rndOrder==0){  //如果顺序为0，开始下落，则开始绘制金币
						if(!coinAttrArr[i].bEnd){   //金币下落（过程一），自由落体运动
							...
							coinAttrArr[i].endSpeed=-Math.abs(coinAttrArr[i].endSpeed*0.9);
            ...
            if ((curTime - last_update) > 100) {
                ...
                if (speed > SHAKE_THRESHOLD) {
					if((curTime-w_curTime)>2000){
						w_curTime!=0 && new Hongbao({density:Math.round(delta)});
					 	w_curTime=curTime;
					}
                }
                last_x = x;
